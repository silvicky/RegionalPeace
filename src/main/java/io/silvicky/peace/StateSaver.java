...
public class StateSaver extends PersistentState {
    public final HashMap<Identifier, HashMap<SpawnGroup, HashMap<Vec3d, Long> > > mp;
    public static final String SAVED="saved";
    public static final Codec<StateSaver> CODEC= RecordCodecBuilder.create((instance)->
            instance.group
                    (
                            Codec.unboundedMap(Identifier.CODEC,Codec.unboundedMap(SpawnGroup.CODEC,Codec.unboundedMap(Vec3d.CODEC,Codec.LONG).xmap(HashMap::new,map->map)).xmap(HashMap::new,map->map)).xmap(HashMap::new,map->map).fieldOf(SAVED).orElse(new HashMap<>()).forGetter(stateSaver -> stateSaver.mp)
                    ).apply(instance,StateSaver::new));
    public StateSaver(HashMap<Identifier, HashMap<SpawnGroup, HashMap<Vec3d, Long> > > mp){this.mp=mp;}
